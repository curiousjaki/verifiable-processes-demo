...
use methods::{VERIFIABLE_PROCESSING_ELF, VERIFIABLE_PROCESSING_ID};
...
impl VerifiableProcessingService for MyVerifiableProcessingService {
    async fn prove_execution(
        ...
        let image_id: [u32; 8] = match &req.image_id == &VERIFIABLE_PROCESSING_ID.to_vec() {
            true => req.image_id.try_into().unwrap(),
            false => {
        ...
        let receipt = prover.prove(env, VERIFIABLE_PROCESSING_ELF).unwrap();
    ...
    async fn verify_execution(
        ...
        let image_id: [u32; 8] = req
            .image_id
            .try_into()
            .expect("Failed to convert Vec<u32> to [u32; 8]");
        //print_type_of(&req.receipt);
        println!(
            "the handed id is {:?} whereas the id of the hash is: {:?}",
            image_id, VERIFIABLE_PROCESSING_ID
        );
        //write_vec_to_file("verify.txt", req.receipt.clone());
        let receipt =
            convert_to_receipt(req.receipt).expect("Somehow the receipt convertion didnt work");
        //println!("{:?}",&receipt);
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    Server::builder()
        .add_service(VerifiableProcessingServiceServer::new(
            verifiable_processing_service,
        ))
        .serve(addr)
        .await?;
